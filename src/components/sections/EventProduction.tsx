...
import type { EventProductionData } from "@/lib/sanity";

interface EventProductionProps {
  data?: EventProductionData;
}

export function EventProduction({ data }: EventProductionProps) {
  const subtitle = data?.subtitle || "イベント演出事業";
        ...
        >
          <motion.div variants={sectionOverline} className="mb-4">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[var(--accent-cta-muted)] border border-[rgba(255,107,0,0.3)]">
              <PartyPopper className="w-3.5 h-3.5 text-[var(--accent-cta)]" />
          ...
          >
            {data?.imageUrl ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[var(--bg-card)] to-[var(--bg-elevated)]">
                <PartyPopper className="w-16 h-16 text-[var(--text-muted)]" />
              </div>
            )}
          </motion.div>
